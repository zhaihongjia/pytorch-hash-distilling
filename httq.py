...
def get_model(learning_rate=0.00001):
    model=DSH(bits)
    model.load_state_dict(torch.load("./models/httq/{}/bits{}.pkl".format(bits,bits)))
...
def train(model, loss, optimization_step_fn,
    ...
    for epoch in range(0, n_epochs):
        ...
        print("save model!------------------------")
        torch.save(model.state_dict(), './models/httq/{}/epoch{}_ttq.pkl'.format(bits,epoch))
...
for bits in {12,24,36,48}:
    ...
    teacher.cuda()
    teacher.train(False)
    model,loss,optimizer=get_model(LEARNING_RATE)
    ...
    # 训练
    train(model, loss, optimization_step_fn,trainloader, testloader, n_epochs=50)
